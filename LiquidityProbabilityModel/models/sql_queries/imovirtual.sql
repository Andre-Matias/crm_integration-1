...
select
    ...
    a.valid_to,
    a.status,
    len(a.title) as title,
    len(a.description) as description,
    a.params,
    a.map_address,
    a.private_business,
...
and server_path = '/h/v-imovirtual-android'
...
and server_path = '/h/v-imovirtual-ios'
and item_id is not null)
UNION ALL
(select
server_date,
...
(select id as ad_id, extract(epoch from created_at_first) as created
from livesync.verticals_ads
where  cast(created_at_first as date)  BETWEEN '2017-06-01' and '2017-12-31') a
JOIN
(select
...
from hydra.verticals_ninja_android_201706
where
server_path = '/h/v-imovirtual-android'
and item_id is not null
...
(select
  ...
  from hydra.verticals_ninja_ios_201706
  where
  server_path = '/h/v-imovirtual-ios'
  and item_id is not null
  and trackpage = 'ad_page')
  UNION ALL
  (select
