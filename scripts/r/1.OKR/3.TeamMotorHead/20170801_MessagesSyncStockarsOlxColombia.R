...
requestDB <- 
  dbSendQuery(
    ...
    AND country_id IN (170) -- 170 Colombia
...
dfStats <- 
  ...
  filter(dayorigin > as.Date(Sys.time())-30)
...
ghQuantityMessagesSynced <-
  ...
  ggtitle("OLX/Stockars.CO - Quantity of Messages synced") + 
...
ghSyncingTime <- 
  ...
  ggtitle("OLX/Stockars.CO - Syncing Time") + 
...
dfUsersNotSincying <-
  ...
  filter(!is.na(message_id), 
         is.na(id_message), 
         dateorigin >= as.Date(Sys.time())-30,
         !is.na(email)
         ) %>%
  group_by(email) %>%
