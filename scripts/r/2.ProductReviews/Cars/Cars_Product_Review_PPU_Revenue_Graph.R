...
bullet.graph <- function(bg.data, bg.title, bg.subtitle, maxgb){
  ...
  gg <- gg + geom_text(data=bg.data[8,],  aes(measure, maxgb, label="target"), color = "#0570b0", vjust = 3, hjust=1,  family="Open Sans")
  gg <- gg + geom_text(data=bg.data[8,], aes(measure, maxgb, label="actual"), color = "#74a9cf", vjust = -2, hjust=1,  family="Open Sans")
