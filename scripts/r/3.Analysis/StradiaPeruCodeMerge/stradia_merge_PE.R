...
setwd("~/verticals-bi/scripts/r/3.Analysis/StradiaPeruCodeMerge")
...
grossDaily<-  ggplot(data=gross, aes(x=day, y=listings)) + geom_bar(stat="identity",fill="#BEC100") +
  ...
  labs(title = "Daily Gross Listings - Stradia PE")
...
# +120% compared with the 4 weeks prior to the merge. 
...
setwd("~/verticals-bi/scripts/r/3.Analysis/StradiaPeruCodeMerge")
vec2<-c("mixpanel_sessions_desktop_pe.csv","mixpanel_sessions_rwd_pe.csv","mixpanel_sessions_android_pe.csv","mixpanel_sessions_ios_pe.csv")
...
visitsWeekly<- visits%>%
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(hjust = 0.5)) +
  scale_x_date(date_breaks  ="1 week") +
  labs(title = "Weekly Visits - Stradia PE", x = "weeks")
...
visitsWeeklyDevice<- visits%>%
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(hjust = 0.5)) +
  scale_x_date(date_breaks  ="1 week") +
  labs(title = "Weekly Visits by Device - Stradia PE", x = "weeks") +
...
ansWeekly<- replies_answers%>%
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1),plot.title = element_text(hjust = 0.5)) +
  scale_x_date(date_breaks  ="1 week") +
  labs(title = "Weekly Answers - Stradia PE", x = "weeks")
...
ansWeeklySource<- replies_answers%>%
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1),plot.title = element_text(hjust = 0.5)) +
  scale_x_date(date_breaks  ="1 week") +
  labs(title = "Weekly Answers by Device - Stradia PE", x = "weeks") +
  facet_wrap(~ device, nrow=1)

plot(ansWeekly)
plot(ansWeeklySource)

#very few answers both before and after the merge, we don't see significant changes.
