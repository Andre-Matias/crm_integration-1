...
jqlQuery <- 
  ...
  .filter(e=>e.name.includes("my_ads_bulk_vas")===true && 
            e.properties.$referring_domain != "www.mystandvirtual.fixeads.com" && 
            e.properties.$referring_domain != "mystandvirtual.fixeads.com"
  )
...
result <- 
  mixpanelJQLQuery(account = mixpanelStandvirtualAccount, 
                   jqlQuery, 
...
dbs <-
  list(
    #c("StandvirtualPT", 3317, "StandvirtualPT")
    #c("StandvirtualPT", 3315, "carspt")
...
dfQueryResults_StandvirtualPT_Stats <-
...
ggplot(df)+
  ...
  ggtitle("Bump Bulk VAS - Started vs Finished Process", subtitle = "STANDVIRTUAL (01/mar - 28/mar/18)")+
...
ggplot(df)+
  ...
  scale_x_datetime(date_breaks = "day", date_labels = "%d\n%b\n%y", limits = c(as.POSIXct("2018-02-28"), as.POSIXct("2018-03-29")))+
  ggtitle("Bump Bulk VAS Convertion Rate", subtitle = "STANDVIRTUAL (01/mar - 28/mar/18)")+
  theme_fivethirtyeight(base_family = "opensans")

ggplot(dfQueryResults_StandvirtualPT_Stats)+
  ...
  scale_y_continuous()+
  scale_x_datetime(date_breaks = "day", date_labels = "%d\n%b\n%y", limits = c(as.POSIXct("2018-02-28"), as.POSIXct("2018-03-29")))+
  ggtitle("Bump Bulk VAS - Total Quantity", subtitle = "STANDVIRTUAL (01/mar - 28/mar/18)")+
  theme_fivethirtyeight(base_family = "opensans")

dfQueryResults3_StandvirtualPT <- 
  dfQueryResults2_StandvirtualPT %>% 
  mutate(Month = as.POSIXct(paste0(substr(Month, 1, 8), "01")))


ggplot(dfQueryResults3_StandvirtualPT)+
  ...
  ggtitle("VAS - Bump Up - Daily Average Quantity", subtitle = "STANDVIRTUAL (01/mar - 28/mar/18)")+
  theme_fivethirtyeight(base_family = "opensans")