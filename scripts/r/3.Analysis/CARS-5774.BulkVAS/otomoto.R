...
result <- 
  mixpanelJQLQuery(account = mixpanelOtomotoAccount, 
                   jqlQuery, 
...
dfQueryResults_OtomotoPL_Stats <-
...
ggplot(df)+
  ...
  ggtitle("Bump Bulk VAS - Started vs Finished Process", subtitle = "OTOMOTO (01/mar - 28/mar/18)")+
...
ggplot(df)+
  ...
  scale_x_datetime(date_breaks = "day", date_labels = "%d\n%b\n%y", limits = c(as.POSIXct("2018-02-28"), as.POSIXct("2018-03-29")))+
  ggtitle("Bump Bulk VAS Convertion Rate", subtitle = "OTOMOTO (01/mar - 28/mar/18)")+
  theme_fivethirtyeight(base_family = "opensans")

ggplot(dfQueryResults_OtomotoPL_Stats)+
  ...
  scale_y_continuous()+
  scale_x_datetime(date_breaks = "day", date_labels = "%d\n%b\n%y", limits = c(as.POSIXct("2018-02-28"), as.POSIXct("2018-03-29")))+
  ggtitle("Bump Bulk VAS - Total Quantity", subtitle = "OTOMOTO (01/mar - 28/mar/18)")+
  theme_fivethirtyeight(base_family = "opensans")

dfQueryResults3_OtomotoPL <- 
  dfQueryResults2_OtomotoPL %>% 
  mutate(Month = as.POSIXct(paste0(substr(Month, 1, 8), "01")))


ggplot(dfQueryResults3_OtomotoPL)+
  ...
  ggtitle("VAS - Bump Up - Daily Average Quantity", subtitle = "OTOMOTO")+
  theme_fivethirtyeight(base_family = "opensans")