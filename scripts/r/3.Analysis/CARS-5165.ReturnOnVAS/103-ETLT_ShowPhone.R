...
s3ReadAndSummarise <-
  function(x,y,z){
    ...
    dfTmp <-  
      ...
      summarise(qtyShowPhone = sum(cm_32399)) %>%
      group_by()%>%
...
file.list <- as.data.frame(
  get_bucket(bucket = s3BucketName,
             prefix = "daniel.rocha/RIX/RAW/showing_phone/ati_574113_showing_")
)

# save feather as RDS ---------------------------------------------------------
lapply(file.list$Key,
       function(x) s3FeatherToRDS(x, "/showing_phone/", "/showing_phone/RDS/"))

# read RDS files --------------------------------------------------------------
file.list.RDS <- as.data.frame(
  get_bucket(
    bucket = s3BucketName,
    prefix = "daniel.rocha/RIX/RAW/showing_phone/RDS/ati_")
)

# summarise RDS By Day and AdID -----------------------------------------------
lapply(file.list.RDS$Key, 
       function(x) s3ReadAndSummarise(x, 
                                      y = "/showing_phone/RDS/ati", 
                                      z = "/showing_phone/RDS/ByDayAdID/ati"))

# read summarised files  ------------------------------------------------------

file.list.RDS.sum <- as.data.frame(
  get_bucket(bucket = s3BucketName,
             prefix = "daniel.rocha/RIX/RAW/showing_phone/RDS/ByDayAdID/")
)
...
s3saveRDS(x = df,
          object = "CARS-5165/ShowingPhoneByDayAdId.RDS",
          bucket = s3BucketName)
