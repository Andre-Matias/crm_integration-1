...
config <- config::get(file = "~/verticals-bi/yml_config/config.yml", 
                      config = Sys.getenv("R_CONFIG_ACTIVE", "stradia_co")
)
...
conDB<- dbConnect(MySQL(), 
                  ...
                  port = ifelse(Sys.info()["nodename"] =="bisb",
                                as.numeric(config$BiServerPort),
                                as.numeric(config$DbPort)),
...
s3saveRDS(x = dfStats,
          object = "colombia_stk_str_messages.RDS",
...
s3saveRDS(x = dfStatsInOut,
          object = "colombia_dfStatsInOut.RDS",
          bucket = "pyrates-data-ocean/GVPI-88")


dfNotSynced <-
  df %>% 
  filter (is.na(created_at) & posted >= "2017-10-03 00:00:00") %>%
  select(id, posted) %>%
  arrange(desc(posted))

s3saveRDS(x = dfNotSynced,
          object = "colombia_NotSynced.RDS",
          bucket = "pyrates-data-ocean/GVPI-88"
)