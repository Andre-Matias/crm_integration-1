...
setwd("~/verticals-bi/scripts/r/3.Analysis/StradiaEcuadorCodeMerge")
...
grossDaily<-  ggplot(data=gross, aes(x=day, y=listings)) + geom_bar(stat="identity",fill="#BEC100") +
  ...
  labs(title = "Daily Gross Listings - Stradia EC")
...
grossWeekly<- gross%>%
  ...
  labs(title = "Weekly Gross Listings - Stradia EC", x = "weeks")
...
setwd("~/verticals-bi/scripts/r/3.Analysis/StradiaEcuadorCodeMerge")
vec2<-c("mixpanel_sessions_desktop_ec.csv","mixpanel_sessions_rwd_ec.csv","mixpanel_sessions_android_ec.csv","mixpanel_sessions_ios_ec.csv")
...
visitsWeekly<- visits%>%
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(hjust = 0.5)) +
  scale_x_date(date_breaks  ="1 week") +
  labs(title = "Weekly Visits - Stradia EC", x = "weeks")
...
visitsWeeklyDevice<- visits%>%
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.title = element_text(hjust = 0.5)) +
  scale_x_date(date_breaks  ="1 week") +
  labs(title = "Weekly Visits by Device - Stradia EC", x = "weeks") +
...
ansWeekly<- replies_answers%>%
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1),plot.title = element_text(hjust = 0.5)) +
  scale_x_date(date_breaks  ="1 week") +
  labs(title = "Weekly Answers - Stradia EC", x = "weeks")
...
ansWeeklySource<- replies_answers%>%
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1),plot.title = element_text(hjust = 0.5)) +
  scale_x_date(date_breaks  ="1 week") +
  labs(title = "Weekly Answers by Device - Stradia EC", x = "weeks") +
  facet_wrap(~ device, nrow=1)
