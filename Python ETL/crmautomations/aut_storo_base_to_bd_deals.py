...
def deletePreviousS3Files(conf_file, keyId, sKeyId):
	print("Deleting S3 storo Files")
	conf = json.load(open(conf_file))

	conn = S3Connection(keyId, sKeyId)
	b = Bucket(conn, 'pyrates-data-ocean')
	for x in b.list(prefix = 'renato-teixeira/deals/storo/'):
		x.delete()
...
def s3_fulldump_deals(client,keyId,sKeyId,bucketName,data_path,category,country):
	...
	aux = 1
	name = "./Reports/aut_storo_base_to_bd_deals_"
	while 1:
		...
		for deal_data in data:
			...
			if 'Lead Create Date' in deal_data.custom_fields:
				deal.lead_create_date = deal_data.custom_fields['Lead Create Date']
			else:
				deal.lead_create_date = ''
		...
		fileName="aut_storo_base_to_bd_deals_" + str(aux).zfill(10) + ".txt.gz"

		full_key_name = os.path.join(data_path+"deals/storo/", fileName)
...
def loadFromS3toRedshift(conf_file,schema,category,country,bucketName,data_path,date,manifest_path):
	...
	if(checkS3FileExists(conf_file,bucketName,str(data_path) + 'deals/storo/') == 'true'):
		print('Loading...')
		cur.execute(
			getCopySql(
				schema, \
				'aut_storo_base_to_bd_deals',
				's3://%(bucketName)s%(data_path)sdeals/storo/' \
					% {
					'bucketName':bucketName,
					'date': date,
					'data_path':data_path},
				's3://%(bucketName)s%(manifest_path)saut_storo_deals_sync_jsonpath.json' \
					% {
					'bucketName':bucketName,
...
def main(conf_file):
	...
	category = json.load(open(conf_file))['category_storo']
