...
with task_insert_rules as (select * from (
	...
	where 1 = 1
				...
				and auto_task.cod_rule = 9 --change this to this task rule 
)
	where 1=1
	and rnumber = 1)
select  (select coalesce(max(cod_auto_task),0) max_cod_auto_task from crm_integration_anlt.t_fac_auto_task) + row_number() over () max_cod_auto_task,
  0 opr_task,
	...
	null dat_task_sync,
	9 cod_rule, --change this to this task rule
	'Twój klient kupuje TOPy pojedynczo. Czas temu zaradzić! (Auto_task_' || (select coalesce(max(cod_auto_task),0) max_cod_auto_task from crm_integration_anlt.t_fac_auto_task) + row_number() over () || ')' content,
