...
with task_insert_rules as (select * from (
	...
	where 1 = 1
				...
				and auto_task.cod_rule = 3 --change this to this task rule
...
select  (select coalesce(max(cod_auto_task),0) max_cod_auto_task from crm_integration_anlt.t_fac_auto_task) + row_number() over () max_cod_auto_task,
  0 opr_task,
	...
	null dat_task_sync,
	3 cod_rule, --change this to this task rule
	'Twojemu klientowi kończą się podbicia. Skontaktuj się z nim! (Auto_task_' || (select coalesce(max(cod_auto_task),0) max_cod_auto_task from crm_integration_anlt.t_fac_auto_task) + row_number() over () || ')' content,
...
from
  (
    ...
    where
      ...
      and lower(packets.index_type) like '%pushup%'
