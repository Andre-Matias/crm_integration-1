...
from munch import * 
import time
import aut_stvpt_base_to_bd_stages_stage_movement
import aut_stvpt_base_to_bd_deals_stage_movement
import threading
...
cur.execute(
			"select "\
              "a.opr_deal, "\
              "case when and to_date(left(a.last_stage_change_at,10),'yyyy-mm-dd') < to_date(sysdate,'yyyy-mm-dd') - 10 then 5374004 "\ #Lost
			  "when to_date(left(c.last_call,10),'yyyy-mm-dd') < 4 then 7284850 "\ # Unqualified
			  "when to_date(left(c.last_call,10),'yyyy-mm-dd') > 4 then 5374004 "\ # Lost
			  "end as opr_stage, "\
            ...
            "from "\
			  ...
			  " a.cod_source_system = 15 "\
			  "group by "\
			  " b.cod_deal "\
			  ") c "\
            "where "\
			  "a.valid_to = 20991231 "\
			  "and a.cod_source_system = 15 "\
...
# 5374004 = Lost: Ver Pipeline do Stage
# 385699  = Lost: Ver Pipeline do Stage
# 7284850 = Unqualified: Ver Pipeline do Stage
# 5374003  = Unqualified: Ver Pipeline do Stage
# 385698  = Unqualified: Ver Pipeline do Stage
