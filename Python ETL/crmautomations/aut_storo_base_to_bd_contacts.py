...
def deletePreviousS3Files(conf_file, keyId, sKeyId):
	...
	for x in b.list(prefix = 'renato-teixeira/contacts/storo/'):
		x.delete()
...
def s3_fulldump_contacts(client,keyId,sKeyId,bucketName,data_path,category,country):
	...
	aux = 1
	name = "./Reports/aut_storo_base_to_bd_contacts_"
	while 1:
		...
		for contact_data in data:
			#if contact_data.updated_at >= '2018-02-26 00:00:00.000000':
			if  str(datetime.strptime(contact_data.updated_at[:10], '%Y-%m-%d')) >= str(datetime.today().date() - timedelta(days=1)):
				...
				contact.created_date = contact_data.created_at
		...
		fileName="aut_storo_base_to_bd_contacts_" + str(aux).zfill(10) + ".txt.gz"

		full_key_name = os.path.join(data_path+"contacts/storo/", fileName)
...
def loadFromS3toRedshift(conf_file,schema,category,country,bucketName,data_path,date,manifest_path):
	...
	if(checkS3FileExists(conf_file,bucketName,str(data_path) + 'contacts/storo/') == 'true'):
		print('Loading...')
		cur.execute(
			getCopySql(
				schema, \
				'aut_storo_base_to_bd_contacts',
				's3://%(bucketName)s%(data_path)scontacts/storo/' \
					% {
					'bucketName':bucketName,
					'date': date,
					'data_path':data_path},
				's3://%(bucketName)s%(manifest_path)saut_storo_contacts_sync_jsonpath.json' \
					% {
					'bucketName':bucketName,
