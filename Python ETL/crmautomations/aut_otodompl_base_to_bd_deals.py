...
def deletePreviousS3Files(conf_file, keyId, sKeyId):
	...
	for x in b.list(prefix = 'crm-automations/deals/otodompl/'):
		x.delete()
...
def s3_fulldump_deals(client,keyId,sKeyId,bucketName,data_path,category,country):
	...
	aux = 1
	name = "./aut_otodompl_base_to_bd_deal_"
	while 1:
		...
		fileName="aut_otodompl_base_to_bd_deal_" + str(aux).zfill(10) + ".txt.gz"

		full_key_name = os.path.join(data_path+"deals/otodompl/", fileName)
...
def loadFromS3toRedshift(conf_file,schema,category,country,bucketName,data_path,date,manifest_path):
	...
	if(checkS3FileExists(conf_file,bucketName,str(data_path) + 'deals/otodompl/') == 'true'):
		print('Loading...')
		cur.execute(
			getCopySql(
				schema, \
				'aut_otodompl_base_to_bd_deal',
				's3://%(bucketName)s%(data_path)sdeals/otodompl/' \
					% {
					'bucketName':bucketName,
					'date': date,
					'data_path':data_path},
				's3://%(bucketName)s%(manifest_path)saut_otodompl_deals_sync_jsonpath.json' \
					% {
					'bucketName':bucketName,
