...
def deletePreviousS3Files(conf_file, keyId, sKeyId):
	print("Deleting S3 otodompl Contacts Files")
	conf = json.load(open(conf_file))

	conn = S3Connection(keyId, sKeyId)
	b = Bucket(conn, 'pyrates-eu-data-ocean')
	for x in b.list(prefix = 'crm-automations/contacts/otodompl/'):
		x.delete()
...
def s3_fulldump_contacts(client,keyId,sKeyId,bucketName,data_path,category,country):
	...
	aux = 1
	name = "./aut_otodompl_base_to_bd_contact_"
	while 1:
		...
		fileName="aut_otodompl_base_to_bd_contact_" + str(aux).zfill(10) + ".txt.gz"

		full_key_name = os.path.join(data_path+"contacts/otodompl/", fileName)
	...
	if(checkS3FileExists(conf_file,bucketName,str(data_path) + 'contacts/otodompl/') == 'true'):
		...
		cur.execute(
			getCopySql(
				...
				's3://%(bucketName)s%(data_path)scontacts/otodompl/' \
					% {
					'bucketName':bucketName,
					'date': date,
					'data_path':data_path},
				's3://%(bucketName)s%(manifest_path)saut_otodompl_contacts_sync_jsonpath.json' \
					% {
					'bucketName':bucketName,
