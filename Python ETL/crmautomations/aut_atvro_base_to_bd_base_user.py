...
def deletePreviousS3Files(conf_file, keyId, sKeyId):
	print("Deleting S3 atvro Files")
	conf = json.load(open(conf_file))

	conn = S3Connection(keyId, sKeyId)
	b = Bucket(conn, 'pyrates-data-ocean')
	for x in b.list(prefix = 'renato-teixeira/base_user/atvro/'):
		x.delete()

@retry(exceptions=Exception, delay=1, tries=10, logger=logger)			
def s3_fulldump_base_user(client,keyId,sKeyId,bucketName,data_path,category,country):
	
	print("Getting base_user data")
	#Iterate for everypage returned by the API
	aux = 1
	name = "./Reports/aut_atvro_base_to_bd_base_user_"
	while 1:
		...
		for user_data in data:
			base_user = Munch()
			base_user.id = user_data.id
			base_user.name = user_data.name
			base_user.country = country
			base_user.category = category
			output.write(json.dumps(base_user,use_decimal=True)+"\n")
		...
		fileName="aut_atvro_base_to_bd_base_user_" + str(aux).zfill(10) + ".txt.gz"

		full_key_name = os.path.join(data_path+"base_user/atvro/", fileName)
...
def loadFromS3toRedshift(conf_file,schema,category,country,bucketName,data_path,date,manifest_path):
	...
	if(checkS3FileExists(conf_file,bucketName,str(data_path) + 'base_user/atvro/') == 'true'):
		print('Loading...')
		cur.execute(
			getCopySql(
				schema, \
				'aut_atvro_base_to_bd_base_user',
				's3://%(bucketName)s%(data_path)sbase_user/atvro/' \
					% {
					'bucketName':bucketName,
					'date': date,
					'data_path':data_path},
				's3://%(bucketName)s%(manifest_path)saut_atvro_base_user_sync_jsonpath.json' \
					% {
					'bucketName':bucketName,
...
### TODO - DELETE S3 PATH BEFORE UNLOADING!!!!
	...
	s3_fulldump_base_user(client,keyId,sKeyId,bucketName,data_path,category,country)
