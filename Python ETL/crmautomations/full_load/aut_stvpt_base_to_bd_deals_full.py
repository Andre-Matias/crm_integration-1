...
def deletePreviousS3Files(conf_file, keyId, sKeyId):
	print("Deleting S3 Stvpt Deals Files")
	conf = json.load(open(conf_file))
...
def s3_fulldump_deals(client,keyId,sKeyId,bucketName,data_path,category,country):
	...
	aux = 1
	name = "./Reports/aut_stvpt_base_to_bd_deals_"
	while 1:
		...
		for deal_data in data:
			...
			if 'NIF' in deal_data.custom_fields:
				...
				deal.nif = ''
			if 'User_ID' in deal_data.custom_fields:
				deal.user_id = deal_data.custom_fields['User_ID']
			else:
				deal.user_id = ''
		...
		fileName="aut_stvpt_base_to_bd_deals_" + str(aux).zfill(10) + ".txt.gz"

		full_key_name = os.path.join(data_path+"deals/stvpt/", fileName)
...
def loadFromS3toRedshift(conf_file,schema,category,country,bucketName,data_path,date,manifest_path):
	...
	if(checkS3FileExists(conf_file,bucketName,str(data_path) + 'deals/stvpt/') == 'true'):
		print('Loading...')
		cur.execute(
			getCopySql(
				schema, \
				'aut_stvpt_base_to_bd_deals',
				's3://%(bucketName)s%(data_path)sdeals/stvpt/' \
					% {
					'bucketName':bucketName,
					'date': date,
					'data_path':data_path},
				's3://%(bucketName)s%(manifest_path)saut_stvpt_deals_sync_jsonpath.json' \
					% {
					'bucketName':bucketName,
