...
from munch import * 
import time
import aut_imopt_base_to_bd_contacts_full
import aut_imopt_base_to_bd_deals_full
import threading
...
print('Start Truncate aut_imopt_base_to_bd_contacts: ' + time.strftime("%H:%M:%S"))
cur.execute("truncate table crm_integration_anlt.aut_imopt_base_to_bd_contacts; ")
print('End Truncate aut_imopt_base_to_bd_contacts: ' + time.strftime("%H:%M:%S"))

print('Start Truncate aut_imopt_base_to_bd_deals: ' + time.strftime("%H:%M:%S"))
cur.execute("truncate table crm_integration_anlt.aut_imopt_base_to_bd_deals; ")
print('End Truncate aut_imopt_base_to_bd_deals: ' + time.strftime("%H:%M:%S"))
...
cur.execute(
			"select "\
              "aut_imopt_base_to_bd_contacts.contacts_id, "\
              "aut_imopt_base_to_bd_contacts.deals_id, "\
              "aut_imopt_base_to_bd_contacts.nif, "\
              "aut_imopt_base_to_bd_contacts.user_id, "\
              "aut_imopt_base_to_bd_contacts.designacao_fiscal, "\
              "aut_imopt_base_to_bd_contacts.fiscal_address, "\
              "aut_imopt_base_to_bd_contacts.email_address, "\
              "aut_imopt_base_to_bd_contacts.natureza "\
            "from "\
              "( "\
                "select "\
                  "aut_imopt_base_to_bd_contacts.id contacts_id, "\
                  "aut_imopt_base_to_bd_deals.id deals_id, "\
                  "aut_imopt_base_to_bd_contacts.nif, "\
                  "aut_imopt_base_to_bd_contacts.user_id, "\
                  "aut_imopt_base_to_bd_contacts.designacao_fiscal, "\
                  "contacts_address.fiscal_address, "\
                  "aut_imopt_base_to_bd_contacts.email_address, "\
                  "replace(replace(replace(aut_imopt_base_to_bd_contacts.natureza,'\\\"',''),'[',''),']','') as natureza "\
                "from "\
                  "crm_integration_anlt.aut_imopt_base_to_bd_contacts, "\
                  "crm_integration_anlt.aut_imopt_base_to_bd_deals, "\
                  "( "\
                    ...
                    "from "\
                      "( "\
                        ...
                        "from "\
                          "crm_integration_anlt.aut_imopt_base_to_bd_contacts ts, "\
                          "( "\
                          "select "\
                            "* "\
                          "from "\
                            ...
                            ") "\
                          "where "\
                          "gen_num between 1 and (select max(regexp_count(replace(fiscal_address,':null',':\\\"null\\\"'), '\\\",\\\"') + 1) from crm_integration_anlt.aut_imopt_base_to_bd_contacts /*where id = 117058292*/) "\
                          ") s "\
                        "where "\
                    ...
                    ") contacts_address "\
                "where  "\
                  "aut_imopt_base_to_bd_contacts.id = aut_imopt_base_to_bd_deals.contacts_id "\
                "and aut_imopt_base_to_bd_contacts.id = contacts_address.id (+) "\
				#"and aut_imopt_base_to_bd_contacts.id = 200682693 "\
				#"and aut_imopt_base_to_bd_deals.id in(49245232,49245233) "\
              ") aut_imopt_base_to_bd_contacts, "\
              "crm_integration_anlt.aut_imopt_base_to_bd_deals "\
            "where "\
              "aut_imopt_base_to_bd_contacts.contacts_id = aut_imopt_base_to_bd_deals.contacts_id (+) "\
              "and aut_imopt_base_to_bd_contacts.deals_id = aut_imopt_base_to_bd_deals.id (+) "\
              "and aut_imopt_base_to_bd_contacts.nif = aut_imopt_base_to_bd_deals.nif (+) "\
              "and aut_imopt_base_to_bd_contacts.user_id = aut_imopt_base_to_bd_deals.user_id (+) "\
              "and aut_imopt_base_to_bd_contacts.designacao_fiscal = aut_imopt_base_to_bd_deals.designacao_fiscal (+) "\
              "and aut_imopt_base_to_bd_contacts.fiscal_address = aut_imopt_base_to_bd_deals.fiscal_address (+) "\
              "and aut_imopt_base_to_bd_contacts.email_address = aut_imopt_base_to_bd_deals.email_address (+) "\
              "and aut_imopt_base_to_bd_contacts.natureza = aut_imopt_base_to_bd_deals.natureza (+) "\
              "and aut_imopt_base_to_bd_deals.id is null;")
