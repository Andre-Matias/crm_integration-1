...
def deletePreviousS3Files(conf_file, keyId, sKeyId):
	...
	for x in b.list(prefix = 'renato-teixeira/contacts/stvpt/'):
		x.delete()
...
def s3_fulldump_contacts(client,keyId,sKeyId,bucketName,data_path,category,country):
	...
	aux = 1
	name = "./aut_stvpt_base_to_bd_contact_"
	while 1:
		...
		cur.execute(
			getCopySql(
				...
				's3://%(bucketName)s%(data_path)scontacts/stvpt/' \
					...
					'date': date,
					'data_path':data_path},
				's3://%(bucketName)s%(manifest_path)saut_stvpt_contacts_sync_jsonpath.json' \
					% {
					'bucketName':bucketName,
