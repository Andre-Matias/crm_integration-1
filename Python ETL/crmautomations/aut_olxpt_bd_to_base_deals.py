...
cur.execute(
			"select "\
              ...
              "case when aut_olxpt_base_to_bd_contact.payment_date = '' then to_char(tO_date(left(replace(replace(aut_olxpt_base_to_bd_contact.last_stage_change_at,'T',' '),'Z',''),10),'yyyy-mm-dd'),'yyyy-mm-dd') else aut_olxpt_base_to_bd_contact.payment_date end payment_date "\
            "from "\
              "( "\
                "select "\
