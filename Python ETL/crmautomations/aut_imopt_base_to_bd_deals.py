...
def deletePreviousS3Files(conf_file, keyId, sKeyId):
	...
	for x in b.list(prefix = 'renato-teixeira/deals/imopt/'):
		x.delete()
...
def s3_fulldump_deals(client,keyId,sKeyId,bucketName,data_path,category,country):
	...
	aux = 1
	name = "./Reports/aut_imopt_base_to_bd_deals_"
	while 1:
		...
		for deal_data in data:
			if  str(datetime.strptime(deal_data.updated_at[:10], '%Y-%m-%d')) >= str(datetime.today().date() - timedelta(days=7)):
				...
				if 'User ID' in deal_data.custom_fields:
					...
					deal.user_id = ''
				if 'Designação Fiscal' in deal_data.custom_fields:
					deal.designacao_fiscal = deal_data.custom_fields['Designação Fiscal']
				else:
					deal.designacao_fiscal = ''
				if 'Fiscal Address' in deal_data.custom_fields:
					deal.fiscal_address = deal_data.custom_fields['Fiscal Address']
				else:
					deal.fiscal_address = ''
				if 'Email Address' in deal_data.custom_fields:
					deal.email_address = deal_data.custom_fields['Email Address']
				else:
					deal.email_address = ''
				if 'Natureza' in deal_data.custom_fields:
					deal.natureza = deal_data.custom_fields['Natureza']
				else:
					deal.natureza = ''
		...
		fileName="aut_imopt_base_to_bd_deals_" + str(aux).zfill(10) + ".txt.gz"
...
def loadFromS3toRedshift(conf_file,schema,category,country,bucketName,data_path,date,manifest_path):
	...
	if(checkS3FileExists(conf_file,bucketName,str(data_path) + 'deals/imopt/') == 'true'):
		...
		cur.execute(
			getCopySql(
				schema, \
				'aut_imopt_base_to_bd_deals',
				's3://%(bucketName)s%(data_path)sdeals/imopt/' \
					% {
					'bucketName':bucketName,
					'date': date,
					'data_path':data_path},
				's3://%(bucketName)s%(manifest_path)saut_imopt_deals_sync_jsonpath.json' \
					% {
					'bucketName':bucketName,
