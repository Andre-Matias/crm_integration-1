...
drop table if exists crm_integration_anlt.tmp_pl_load_ad_md5_step2;

create table crm_integration_anlt.tmp_pl_load_ad_md5_step2
distkey(opr_ad)
sortkey(opr_ad, opr_source_system, opr_category, opr_city, opr_atlas_user, opr_ad_status, opr_offer_seek, opr_solr_archive_status, opr_solr_status, opr_rmoderation_status, opr_source, opr_new_used)
as
SELECT
...
analyze crm_integration_anlt.tmp_pl_load_ad_md5_step2;
	
	--$$$
	
drop table if exists crm_integration_anlt.tmp_pl_load_ad;

create table crm_integration_anlt.tmp_pl_load_ad 
distkey(cod_ad)
...
as
 ...
 select
  ...
  from
	crm_integration_anlt.tmp_pl_load_ad_md5_step2 source_table,
...
analyze crm_integration_anlt.tmp_pl_load_ad;
	
	--$$$ 
	
delete from crm_integration_anlt.t_lkp_ad
using crm_integration_anlt.tmp_pl_load_ad
where 
	tmp_pl_load_ad.dml_type = 'I' 
	and t_lkp_ad.cod_ad = tmp_pl_load_ad.cod_ad
...
set valid_to = (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 2 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_ad')
from crm_integration_anlt.tmp_pl_load_ad source
...
insert into crm_integration_anlt.t_lkp_ad
    select
		...
		cod_execution
    from
      crm_integration_anlt.tmp_pl_load_ad
    where
      dml_type in ('U','I');
