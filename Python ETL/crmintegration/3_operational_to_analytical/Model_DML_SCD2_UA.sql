...
create temp table tmp_ua_load_resource_type
as
select
  ...
  from
    (
      ...
      from
        (
          select distinct resource_type from crm_integration_stg.stg_ua_d_base_calls where resource_type is not null
          union
          select distinct resource_type from crm_integration_stg.stg_ua_d_base_tags where resource_type is not null
          union
          select distinct resource_type from crm_integration_stg.stg_ua_d_base_sources where resource_type is not null
          union
          select distinct resource_type from crm_integration_stg.stg_ua_d_base_tasks where resource_type is not null
        ) a,
        (
          select
            rel_integr_proc.dat_processing,
...
analyze tmp_ua_load_resource_type;
...
create temp table tmp_ua_load_industry
as
select
...
from
    (
      ...
      from
        (
          select distinct base_account_country+base_account_category opr_source_system,industry from crm_integration_stg.stg_ua_d_base_contacts where industry is not null
          union
          select distinct base_account_country+base_account_category opr_source_system,industry from crm_integration_stg.stg_ua_d_base_leads where industry is not null
        ) a,
...
insert into crm_integration_anlt.t_lkp_industry
    select
      ...
      opr_industry,
      dsc_industry,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 3 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_industry') valid_from, 
      20991231 valid_to,
      cod_source_system,
	  cod_execution
    from
      tmp_ua_load_industry
    where
      dml_type = 'I';
...
create temp table tmp_ua_load_lead_status
as
select
...
from
    (
      ...
      from
        (
          select distinct base_account_country+base_account_category opr_source_system,status from crm_integration_stg.stg_ua_d_base_leads where status is not null
        ) a,
        crm_integration_anlt.t_lkp_source_system b,
        (
          select
...
analyze tmp_ua_load_lead_status;
...
insert into crm_integration_anlt.t_lkp_lead_status
    select
      ...
      opr_lead_status,
      dsc_lead_status,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 3 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_lead_status') valid_from, 
      20991231 valid_to,
      cod_source_system,
	  cod_execution
    from
      tmp_ua_load_lead_status
    where
      dml_type = 'I';
...
create temp table tmp_ua_load_product
as
select
...
from
    (
      ...
      from
        (
          select distinct base_account_country+base_account_category opr_source_system,sku from crm_integration_stg.stg_ua_d_base_line_items where sku is not null
        ) a,
        crm_integration_anlt.t_lkp_source_system b,
        (
          select
...
insert into crm_integration_anlt.t_lkp_product
    select
      ...
      opr_sku,
      dsc_sku,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 3 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_product') valid_from, 
      20991231 valid_to,
      cod_source_system,
	  cod_execution
    from
...
create temp table tmp_ua_load_currency
as
select
  ...
  from
    (
      ...
      from
        (
          select distinct currency from crm_integration_stg.stg_ua_d_base_line_items where currency is not null
        ) a,
        (
          select
            rel_integr_proc.dat_processing,
...
insert into crm_integration_anlt.t_lkp_currency
    select
      ...
      opr_currency,
      dsc_currency,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 3 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_currency') valid_from, 
      20991231 valid_to,
      cod_source_system,
	  cod_execution
    from
      tmp_ua_load_currency
    where
      dml_type = 'I';
  ...
  from
	...
	(
      SELECT
        ...
        scai_execution.dat_processing
      FROM
        crm_integration_stg.stg_ua_d_base_users,
        (
          select
            rel_integr_proc.dat_processing,
            max(fac.cod_execution) cod_execution
  ...
  from
	...
	(
      SELECT DISTINCT
		...
		scai_execution.dat_processing
      FROM
        crm_integration_stg.stg_ua_d_base_contacts,
        (
          select
            rel_integr_proc.dat_processing,
            max(fac.cod_execution) cod_execution
      ...
      from
        (
          select
            ...
            scai_execution.dat_processing
          from
            crm_integration_stg.stg_ua_d_base_calls a,
            crm_integration_anlt.t_lkp_source_system b,
            (
              select
                rel_integr_proc.dat_processing,
...
-- Updating BASE CONTACT - Autovit (vertical does not exist yet for Ukraine)
update crm_integration_anlt.t_lkp_contact
set cod_atlas_user = source.cod_atlas_user
from
  (
...
-- Updating BASE CONTACT - Storia (vertical does not exist yet for Ukraine)
update crm_integration_anlt.t_lkp_contact
set cod_atlas_user = source.cod_atlas_user
from
  (
