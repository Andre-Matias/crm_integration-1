...
create temp table tmp_bg_load_resource_type
as
select
  ...
  from
    (
      ...
      from
        (
          select distinct resource_type from crm_integration_stg.stg_bg_d_base_calls where resource_type is not null
          union
          select distinct resource_type from crm_integration_stg.stg_bg_d_base_tags where resource_type is not null
          union
          select distinct resource_type from crm_integration_stg.stg_bg_d_base_sources where resource_type is not null
          union
          select distinct resource_type from crm_integration_stg.stg_bg_d_base_tasks where resource_type is not null
        ) a,
        (
          select
            rel_integr_proc.dat_processing,
...
analyze tmp_bg_load_resource_type;
...
create temp table tmp_bg_load_industry
as
select
...
from
    (
      ...
      from
        (
          select distinct base_account_country+base_account_category opr_source_system,industry from crm_integration_stg.stg_bg_d_base_contacts where industry is not null
          union
          select distinct base_account_country+base_account_category opr_source_system,industry from crm_integration_stg.stg_bg_d_base_leads where industry is not null
        ) a,
...
insert into crm_integration_anlt.t_lkp_industry
    select
      ...
      opr_industry,
      dsc_industry,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_industry') valid_from, 
      20991231 valid_to,
      cod_source_system,
	  cod_execution
    from
      tmp_bg_load_industry
    where
      dml_type = 'I';
...
create temp table tmp_bg_load_lead_status
as
select
...
from
    (
      ...
      from
        (
          select distinct base_account_country+base_account_category opr_source_system,status from crm_integration_stg.stg_bg_d_base_leads where status is not null
        ) a,
        crm_integration_anlt.t_lkp_source_system b,
        (
          select
...
analyze tmp_bg_load_lead_status;
...
insert into crm_integration_anlt.t_lkp_lead_status
    select
      ...
      opr_lead_status,
      dsc_lead_status,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_lead_status') valid_from, 
      20991231 valid_to,
      cod_source_system,
	  cod_execution
    from
      tmp_bg_load_lead_status
    where
      dml_type = 'I';
...
create temp table tmp_bg_load_product
as
select
...
from
    (
      ...
      from
        (
          select distinct base_account_country+base_account_category opr_source_system,sku from crm_integration_stg.stg_bg_d_base_line_items where sku is not null
        ) a,
        crm_integration_anlt.t_lkp_source_system b,
        (
          select
...
insert into crm_integration_anlt.t_lkp_product
    select
      ...
      opr_sku,
      dsc_sku,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_product') valid_from, 
      20991231 valid_to,
      cod_source_system,
	  cod_execution
    from
...
create temp table tmp_bg_load_currency
as
select
  ...
  from
    (
      ...
      from
        (
          select distinct currency from crm_integration_stg.stg_bg_d_base_line_items where currency is not null
        ) a,
        (
          select
            rel_integr_proc.dat_processing,
...
insert into crm_integration_anlt.t_lkp_currency
    select
      ...
      opr_currency,
      dsc_currency,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_currency') valid_from, 
      20991231 valid_to,
      cod_source_system,
	  cod_execution
    from
      tmp_bg_load_currency
    where
      dml_type = 'I';
...
create temp table tmp_bg_load_source
as
select
  ...
  from
    (
      ...
      from
        (
          select distinct source from db_atlas_verticals.ads, crm_integration_anlt.t_lkp_source_system where source is not null and livesync_dbname = opr_source_system and cod_country = 5
          union
          select distinct source from db_atlas.olxbg_ads where source is not null
          union
          select distinct source from db_atlas_verticals.payment_session, crm_integration_anlt.t_lkp_source_system where source is not null and livesync_dbname = opr_source_system and cod_country = 5
          union
          select distinct source from db_atlas.olxbg_payment_session where source is not null
          union
          select distinct source from db_atlas_verticals.answers, crm_integration_anlt.t_lkp_source_system where source is not null and livesync_dbname = opr_source_system and cod_country = 5
          union
          select distinct source from db_atlas.olxbg_answers where source is not null
        ) a,
        (
          select
            rel_integr_proc.dat_processing,
...
insert into crm_integration_anlt.t_lkp_source
    select
      ...
      opr_source,
      dsc_source,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_source') valid_from, 
      20991231 valid_to,
      cod_source_system,
	  cod_execution
    from
...
create temp table tmp_bg_load_event
as
select
  ...
  from
    (
      ...
      from
        (
          select distinct action_type event_type from crm_integration_stg.stg_bg_hydra_web where action_type is not null
          union
          select distinct trackname event_type from crm_integration_stg.stg_bg_hydra_verticals_web where trackname is not null
        ) a,
        (
          select
            rel_integr_proc.dat_processing,
...
insert into crm_integration_anlt.t_lkp_event
    select
      ...
      opr_event,
      dsc_event,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_event') valid_from, 
      20991231 valid_to,
      cod_source_system,
	  cod_execution
    from
...
create temp table tmp_bg_load_base_source 
  ...
  from
	...
	(
      SELECT
        ...
        scai_execution.dat_processing
      FROM
        crm_integration_stg.stg_bg_d_base_sources,
        (
          select
            rel_integr_proc.dat_processing,
            max(fac.cod_execution) cod_execution
	...
	and lkp_source_system.cod_country = 5 -- Bulgaria
	) source_table,
...
from tmp_bg_load_base_source source
    ...
    select
      case
        when dml_type = 'I' then case when valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc
														where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_base_source')
      ...
      dsc_base_source,
      cod_source_system,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_base_source') valid_from, 
      20991231 valid_to,
      created_at,
      updated_at,
      cod_resource_type,
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(updated_at) from tmp_bg_load_base_source),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
...
create temp table tmp_bg_load_base_user 
distkey(cod_source_system)
...
as
  ...
  from
	...
	(
      SELECT
        ...
        scai_execution.dat_processing
      FROM
        crm_integration_stg.stg_bg_d_base_users,
        (
          select
            rel_integr_proc.dat_processing,
            max(fac.cod_execution) cod_execution
    ...
    crm_integration_anlt.t_lkp_source_system lkp_source_system
	where source.opr_source_system = lkp_source_system.opr_source_system
	and lkp_source_system.cod_country = 5
	) source_table,
  ...
  where
	...
	and source_table.cod_source_system = target_base_user_responsible.cod_source_system (+); -- Bulgaria

analyze tmp_bg_load_base_user;


	
delete from crm_integration_anlt.t_lkp_base_user
using tmp_bg_load_base_user
where 
	tmp_bg_load_base_user.dml_type = 'I' 
	and t_lkp_base_user.opr_base_user = tmp_bg_load_base_user.opr_base_user 
	and t_lkp_base_user.valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_base_user');
...
set valid_to = (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_base_user') 
from tmp_bg_load_base_user source
...
insert into crm_integration_anlt.t_lkp_base_user
    select
      case
        when dml_type = 'I' then case when valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc
														where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_base_user')
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(updated_at) from tmp_bg_load_base_user),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
...
create temp table tmp_bg_load_task 
distkey(cod_source_system)
  ...
  from
	...
	(
      SELECT
        ...
        scai_execution.dat_processing
      FROM
        crm_integration_stg.stg_bg_d_base_tasks,
        (
          select
            rel_integr_proc.dat_processing,
            max(fac.cod_execution) cod_execution
...
insert into crm_integration_anlt.t_lkp_task
    select
      case
        when dml_type = 'I' then case when valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc
														where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_task')
									then cod_task else max_cod + new_cod end
        when dml_type = 'U' then cod_task
      end cod_task,
      opr_task,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_task') valid_from, 
      20991231 valid_to,
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(updated_at) from tmp_bg_load_task),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
...
create temp table tmp_bg_load_call_outcome 
distkey(cod_source_system)
  ...
  from
	...
	(
      SELECT
        ...
        scai_execution.dat_processing
      FROM
        crm_integration_stg.stg_bg_d_base_call_outcomes,
        (
          select
            rel_integr_proc.dat_processing,
            max(fac.cod_execution) cod_execution
...
insert into crm_integration_anlt.t_lkp_call_outcome
    select
      case
        when dml_type = 'I' then case when valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc
														where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_call_outcome')
      ...
      dsc_call_outcome,
      cod_source_system,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_call_outcome') valid_from, 
      20991231 valid_to,
      cod_base_user_creator,
      created_at,
      updated_at,
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(updated_at) from tmp_bg_load_call_outcome),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
  ...
  from
	...
	(
      SELECT DISTINCT
		...
		scai_execution.dat_processing
      FROM
        crm_integration_stg.stg_bg_d_base_contacts,
        (
          select
            rel_integr_proc.dat_processing,
            max(fac.cod_execution) cod_execution
...
insert into crm_integration_anlt.t_lkp_contact
    select
      case
        when dml_type = 'I' then case when valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc
														where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_contact')
      ...
      dsc_contact,
      cod_source_system,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_contact') valid_from, 
      20991231 valid_to,
...
insert into crm_integration_anlt.t_lkp_custom_field
    select
      ...
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_custom_field') valid_from, 
      20991231 valid_to,
	  cod_execution
    from
      tmp_bg_load_custom_field
    where
      dml_type = 'I';
...
create temp table tmp_bg_rel_contact_custom_field as
  select
    source.cod_contact,
  ...
  from
    (
      select
        ...
        scai_execution.cod_execution
      from
        tmp_bg_contact_custom_field tmp_cf,
...
analyze tmp_bg_rel_contact_custom_field;
	

	
update crm_integration_anlt.t_rel_contact_custom_field
set valid_to = (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_rel_contact_custom_field') 
from tmp_bg_rel_contact_custom_field source
...
insert into crm_integration_anlt.t_rel_contact_custom_field
  select
    ...
    cod_source_system,
    custom_field_value,
    (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_rel_contact_custom_field') valid_from, 
    20991231 valid_to,
	cod_execution
  from
    tmp_bg_rel_contact_custom_field
  where
    dml_type in ('I','U');
...
create temp table tmp_bg_load_lead 
distkey(cod_source_system)
  ...
  from
	...
	(
      SELECT
        ...
        scai_execution.dat_processing
      FROM
        crm_integration_stg.stg_bg_d_base_leads,
        (
          select
            rel_integr_proc.dat_processing,
            max(fac.cod_execution) cod_execution
...
insert into crm_integration_anlt.t_lkp_lead
    select
      case
        when dml_type = 'I' then case when valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc
														where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_lead')
      ...
      dsc_lead,
      cod_source_system,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_lead') valid_from, 
      20991231 valid_to,
      first_name,
      last_name,
      cod_base_user_owner,
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(updated_at) from tmp_bg_load_lead),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
...
create temp table tmp_bg_load_loss_reason 
distkey(cod_source_system)
  ...
  from
	...
	(
      SELECT
        ...
        scai_execution.dat_processing
	  FROM
        crm_integration_stg.stg_bg_d_base_loss_reasons,
        (
          select
            rel_integr_proc.dat_processing,
            max(fac.cod_execution) cod_execution
...
analyze tmp_bg_load_loss_reason;
	

	
delete from crm_integration_anlt.t_lkp_loss_reason
using tmp_bg_load_loss_reason
where 
	tmp_bg_load_loss_reason.dml_type = 'I' 
	and t_lkp_loss_reason.opr_loss_reason = tmp_bg_load_loss_reason.opr_loss_reason 
	and t_lkp_loss_reason.valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_loss_reason');


	
update crm_integration_anlt.t_lkp_loss_reason
set valid_to = (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_loss_reason') 
from tmp_bg_load_loss_reason source
...
insert into crm_integration_anlt.t_lkp_loss_reason
	select
	  case
		when dml_type = 'I' then case when valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc
														where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_loss_reason')
	  ...
	  dsc_loss_reason,
	  cod_source_system,
	  (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_loss_reason') valid_from, 
	  20991231 valid_to,
	  cod_base_user_creator,
	  created_at,
	  updated_at,
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(updated_at) from tmp_bg_load_loss_reason),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
...
create temp table tmp_bg_load_pipeline 
distkey(cod_source_system)
  ...
  from
	...
	(
      SELECT
        ...
        scai_execution.dat_processing
	  FROM
        crm_integration_stg.stg_bg_d_base_pipelines,
        (
          select
            rel_integr_proc.dat_processing,
            max(fac.cod_execution) cod_execution
...
insert into crm_integration_anlt.t_lkp_pipeline
    select
      case
        when dml_type = 'I' then case when valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc
														where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_pipeline')
      ...
      dsc_pipeline,
      cod_source_system,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_pipeline') valid_from, 
      20991231 valid_to,
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(updated_at) from tmp_bg_load_pipeline),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
...
create temp table tmp_bg_load_stage 
distkey(cod_source_system)
  ...
  from
	...
	(
      SELECT
        ...
        scai_execution.dat_processing
      FROM
        crm_integration_stg.stg_bg_d_base_stages,
        (
          select
            rel_integr_proc.dat_processing,
            max(fac.cod_execution) cod_execution
...
analyze tmp_bg_load_stage;
	

	
delete from crm_integration_anlt.t_lkp_stage
using tmp_bg_load_stage
where 
	tmp_bg_load_stage.dml_type = 'I' 
	and t_lkp_stage.opr_stage = tmp_bg_load_stage.opr_stage
	and t_lkp_stage.valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_stage');


	
update crm_integration_anlt.t_lkp_stage
set valid_to = (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_stage') 
from tmp_bg_load_stage source
...
insert into crm_integration_anlt.t_lkp_stage
    select
      case
        when dml_type = 'I' then case when valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc
														where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_stage')
      ...
      dsc_stage,
      cod_source_system,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_stage') valid_from, 
      20991231 valid_to,
      position,
      likelihood,
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(updated_at) from tmp_bg_load_stage),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
...
create temp table tmp_bg_load_deals 
distkey(cod_source_system)
      ...
      from
        (
          select
            ...
            scai_execution.dat_processing
          from
            crm_integration_stg.stg_bg_d_base_deals,
            (
              select
                rel_integr_proc.dat_processing,
                max(fac.cod_execution) cod_execution
...
analyze tmp_bg_load_deals;
	

	
delete from crm_integration_anlt.t_lkp_deal
using tmp_bg_load_deals
where 
	tmp_bg_load_deals.dml_type = 'I' 
	and t_lkp_deal.opr_deal = tmp_bg_load_deals.opr_deal
	and t_lkp_deal.valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_deal');


	
update crm_integration_anlt.t_lkp_deal
set valid_to = (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_deal') 
from tmp_bg_load_deals source
...
insert into crm_integration_anlt.t_lkp_deal
    select
      case
        when dml_type = 'I' then case when valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc
														where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_deal')
      ...
      dsc_deal,
      cod_source_system,
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_deal') valid_from, 
      20991231 valid_to,
	  last_activity_at,
	  cod_contact,
	  cod_base_source,
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(updated_at) from tmp_bg_load_deals),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
...
create temp table tmp_bg_load_calls 
distkey(cod_source_system)
sortkey(opr_call)
as
      ...
      from
        (
          select
            ...
            scai_execution.dat_processing
          from
            crm_integration_stg.stg_bg_d_base_calls a,
            crm_integration_anlt.t_lkp_source_system b,
            (
              select
                rel_integr_proc.dat_processing,
...
using tmp_bg_load_calls
where crm_integration_anlt.t_fac_call.opr_call = tmp_bg_load_calls.opr_call
and tmp_bg_load_calls.dml_type = 'U';
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(updated_at) from tmp_bg_load_calls),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
...
create temp table tmp_bg_load_orders 
distkey(cod_source_system)
sortkey(opr_order)
as
  select
  ...
  from
    (
      SELECT
        ...
        scai_execution.dat_processing
      FROM
        crm_integration_stg.stg_bg_d_base_orders a,
        crm_integration_anlt.t_lkp_source_system b,
        (
          select
            rel_integr_proc.dat_processing,
...
using tmp_bg_load_orders
where crm_integration_anlt.t_fac_order.opr_order = tmp_bg_load_orders.opr_order
and tmp_bg_load_orders.dml_type = 'U';
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(updated_at) from tmp_bg_load_orders),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
...
create temp table tmp_bg_load_order_line_items 
distkey(cod_source_system)
  ...
  from
    (
      SELECT
        ...
        scai_execution.dat_processing
      FROM
        crm_integration_stg.stg_bg_d_base_line_items a,
        crm_integration_anlt.t_lkp_source_system b,
        (
          select
            rel_integr_proc.dat_processing,
...
analyse tmp_bg_load_order_line_items;
...
insert into crm_integration_anlt.t_hst_order_line_item
    ...
    from
      crm_integration_anlt.t_fac_order_line_item target,
      tmp_bg_load_order_line_items source
    where
...
using tmp_bg_load_order_line_items
where crm_integration_anlt.t_fac_order_line_item.opr_order_line_item=tmp_bg_load_order_line_items.opr_order_line_item
and tmp_bg_load_order_line_items.dml_type = 'U';
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(updated_at) from tmp_bg_load_order_line_items),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
...
create temp table tmp_bg_load_paidad_index 
distkey(cod_source_system)
      ...
      (
        select
          ...
          type opr_paidad_index_type,
           ...
           scai_execution.cod_execution
        from
           crm_integration_stg.stg_bg_db_atlas_verticals_paidads_indexes a,
           crm_integration_anlt.t_lkp_source_system b,
          (
            select
              max(fac.cod_execution) cod_execution
        ...
        where
           ...
           and b.cod_country = 5 -- Bulgaria
		   --and 1 = 0
        union all
        select
           id opr_paidad_index,
           description dsc_paidad_index,
           'olxbg' opr_source_system,
		   operation_type,
		   operation_timestamp,
           code paidad_index_code,
          type opr_paidad_index_type,
           ...
           scai_execution.cod_execution
        from
          crm_integration_stg.stg_bg_db_atlas_olxbg_paidads_indexes,
          (
            select
              max(fac.cod_execution) cod_execution
            from
...
insert into crm_integration_anlt.t_lkp_paidad_index
    select
      ...
      (select rel_integr_proc.dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_paidad_index') valid_from, 
      20991231 valid_to,
...
from tmp_bg_load_paidad_index source, crm_integration_anlt.t_lkp_paidad_index lkp, crm_integration_anlt.t_lkp_source_system ss
where coalesce(source.opr_paidad_index_related,-1) = lkp.opr_paidad_index
and source.cod_source_system = ss.cod_source_system
and ss.cod_country = 5
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(operation_timestamp) from tmp_bg_load_paidad_index),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
...
create temp table tmp_bg_load_atlas_user 
distkey(cod_source_system)
...
insert into crm_integration_anlt.t_lkp_atlas_user
	 select
       case
        when dml_type = 'I' then case when valid_from = (select dat_processing from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc
														where rel_integr_proc.cod_process = proc.cod_process and rel_integr_proc.cod_country = 5 and rel_integr_proc.cod_integration = 30000 and rel_integr_proc.ind_active = 1 and proc.dsc_process_short = 't_lkp_atlas_user')
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(operation_timestamp) from tmp_bg_load_atlas_user),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
...
and cod_source_system in (select cod_source_system from crm_integration_anlt.t_lkp_source_system where cod_country = 5);
...
create temp table tmp_bg_load_answer_step1_outgoing
...
(
  select
    ...
    scai_execution.cod_execution
  from
    crm_integration_stg.stg_bg_db_atlas_verticals_answers a,
    crm_integration_anlt.t_lkp_source_system b,
    (
		select
		  max(fac.cod_execution) cod_execution
  ...
  select
    ...
    scai_execution.cod_execution
  from
    crm_integration_stg.stg_bg_db_atlas_olxbg_answers,
    (select cod_source_system from crm_integration_anlt.t_lkp_source_system
      where opr_source_system = 'olxbg'),
    (
		select
		  max(fac.cod_execution) cod_execution
		from
...
analyze tmp_bg_load_answer_step1_outgoing;



create temp table tmp_bg_load_answer_step2_outgoing
distkey(opr_ad)
...
as
  ...
  from
    tmp_bg_load_answer_step1_outgoing source_table,
...
analyze tmp_bg_load_answer_step2_outgoing;
 	

	
insert into crm_integration_anlt.t_hst_answer_outgoing
select * from crm_integration_anlt.t_fac_answer_outgoing
where crm_integration_anlt.t_fac_answer_outgoing.opr_answer in (select distinct opr_answer from tmp_bg_load_answer_step2_outgoing)
and crm_integration_anlt.t_fac_answer_outgoing.cod_source_system in (select distinct cod_source_system from tmp_bg_load_answer_step2_outgoing);
...
insert into crm_integration_anlt.t_fac_answer_outgoing
    select
	  ...
	  cod_execution
    from
      tmp_bg_load_answer_step2_outgoing;
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(operation_timestamp) from tmp_bg_load_answer_step2_outgoing),last_processing_datetime)
from crm_integration_anlt.t_lkp_scai_process proc 
where t_rel_scai_integration_process.cod_process = proc.cod_process
and t_rel_scai_integration_process.cod_status = 2
and t_rel_scai_integration_process.cod_country = 5
...
create temp table tmp_bg_load_answer_step1_incoming
...
analyze tmp_bg_load_answer_step1_incoming;



create temp table tmp_bg_load_answer_step2_incoming
distkey(opr_ad)
sortkey(opr_ad, cod_source_system)
as
  select
    ...
    row_number() over (order by source_table.opr_answer desc) new_cod
  from
    tmp_bg_load_answer_step1_incoming source_table,
...
analyze tmp_bg_load_answer_step2_incoming;
		


insert into crm_integration_anlt.t_hst_answer_incoming
select * from crm_integration_anlt.t_fac_answer_incoming
where crm_integration_anlt.t_fac_answer_incoming.opr_answer in (select distinct opr_answer from tmp_bg_load_answer_step2_incoming)
and crm_integration_anlt.t_fac_answer_incoming.cod_source_system in (select distinct cod_source_system from tmp_bg_load_answer_step2_incoming);
...
insert into crm_integration_anlt.t_fac_answer_incoming
    select
	  ...
	  cod_execution
    from
      tmp_bg_load_answer_step2_incoming;
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(operation_timestamp) from tmp_bg_load_answer_step2_incoming),last_processing_datetime)
from crm_integration_anlt.t_lkp_scai_process proc
where t_rel_scai_integration_process.cod_process = proc.cod_process
and t_rel_scai_integration_process.cod_status = 2
and t_rel_scai_integration_process.cod_country = 5
...
create temp table tmp_bg_load_web_step1
distkey(opr_ad)
      ...
      from
        (
          SELECT
            ...
            scai_execution.cod_execution
          FROM
            crm_integration_stg.stg_bg_hydra_verticals_web a,
            crm_integration_anlt.t_lkp_source_system b,
            (
              select
                max(fac.cod_execution) cod_execution
          ...
          SELECT
            ...
            scai_execution.cod_execution
          FROM
            crm_integration_stg.stg_bg_hydra_web a,
            crm_integration_anlt.t_lkp_source_system b,
            (
              select
                max(fac.cod_execution) cod_execution
...
analyze tmp_bg_load_web_step1;


		
create temp table tmp_bg_load_web
distkey(dat_event)
...
as
  select
	...
	source_table.cod_execution
  from
    tmp_bg_load_web_step1 source_table,
...
analyze tmp_bg_load_web;


	
delete from crm_integration_anlt.t_fac_web
where crm_integration_anlt.t_fac_web.dat_event in (select distinct dat_event from tmp_bg_load_web)
and crm_integration_anlt.t_fac_web.cod_source_system in (select distinct cod_source_system from tmp_bg_load_web);
...
update crm_integration_anlt.t_rel_scai_integration_process
set cod_status = 1, -- Ok
last_processing_datetime = coalesce((select max(server_date_day_datetime) from tmp_bg_load_web),last_processing_datetime)
/*from
  (
    select proc.cod_process, rel_country_integr.dat_processing, rel_country_integr.cod_country, rel_country_integr.execution_nbr, rel_country_integr.cod_status, rel_country_integr.cod_integration
    from crm_integration_anlt.t_lkp_scai_process proc, crm_integration_anlt.t_rel_scai_integration_process rel_integr_proc, crm_integration_anlt.t_rel_scai_country_integration rel_country_integr
