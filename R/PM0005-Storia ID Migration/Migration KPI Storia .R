...
AdpageVisitst <- data.frame(get_ga(profileId = ids, start.date = today,
                                   end.date = "today", metrics = c("ga:sessions"), 
                                   dimensions = c("ga:date"),  sort = NULL, filters = "ga:pagePath=@/ad",
                                   segment = NULL, samplingLevel = NULL, start.index = NULL,
                                   max.results = NULL, include.empty.rows = NULL, fetch.by = "day", ga_tokenGA))
...
cmd_storia <- 'ssh -i /Users/pedromatos/Documents/bi_id_rsa biuser@52.74.90.117 -p 10022 -L 10007:172.50.21.59:3306 -N'
...
UsersStorTotal$Date <- as.Date(UsersStorTotal$Date, tz = "CET")
...
AdpageVisitst <- rbind(AdpageVisitsy,AdpageVisitst)
colnames(AdpageVisitst) <- c("Date","Ad Page Visits")
AdpageVisitst$Date <- as.Date(AdpageVisitst$Date, tz = "CET")
#AdpageVisits$Date <- as.Date(AdpageVisits$Date, tz = "CET")
AdpageVisits <- AdpageVisits[!(AdpageVisits$Date %in% AdpageVisitst$Date),]
AdpageVisits <- rbind(AdpageVisits,AdpageVisitst)
